...
///
/// This function uploads an IPA to App Store Connect and optionally
/// distributes it to beta testers.
///
/// ## Example
/// ```swift
...
///
/// - Parameters:
///   - ipa: Path to the IPA file. If nil, looks in artifacts from previous gym action.
///   - appId: App Store Connect app ID. If nil, reads from APP_STORE_APP_ID environment.
///   - changelog: Release notes (what's new) for TestFlight testers.
///   - groups: Names of beta groups to distribute to.
///   - skipWaitingForProcessing: Skip waiting for Apple to process the build (default: true).
    ...
    AnyAction(
        PilotAction(
            options: PilotOptions(
                ...
                changelog: changelog,
                groups: groups,
                skipWaitingForProcessing: skipWaitingForProcessing
            )
        )
    )
}

/// Alias for pilot() - uploads a build to TestFlight.
///
/// This is an alternative name for the pilot() action for developers
/// who prefer a more descriptive function name.
///
/// - Parameters:
///   - ipa: Path to the IPA file.
///   - appId: App Store Connect app ID.
///   - changelog: Release notes for TestFlight testers.
///   - groups: Names of beta groups to distribute to.
///   - skipWaitingForProcessing: Skip waiting for build processing.
/// - Returns: A type-erased action that can be executed in a lane.
