...
///
/// This function downloads and installs certificates and profiles from a Git
/// repository for the specified certificate type and app identifier.
///
/// ## Example
/// ```swift
/// lane("beta") {
...
///
/// - Parameters:
///   - type: The certificate type to sync (development, distribution, adhoc, appstore).
///   - readonly: Whether to run in readonly mode (don't create new certificates). Defaults to false.
///   - appIdentifier: App identifier to sync profiles for. If nil, syncs all profiles.
///   - teamId: Team ID for code signing. If nil, reads from MATCH_TEAM_ID environment.
///   - gitUrl: Git repository URL. If nil, reads from MATCH_GIT_URL environment.
///   - forceForNewDevices: Force profile regeneration when new devices are added. Defaults to false.
///   - branch: Git branch to use. Defaults to "master".
///   - password: Encryption password. If nil, reads from MATCH_PASSWORD environment.
