...
/// The root configuration type for a Swiftlane manifest.
///
/// A Swiftlanefile defines the lanes available for execution in a CI/CD pipeline.
/// It can be serialized to JSON for inspection and tooling purposes.
///
/// ## Example
/// ```swift
/// let manifest = Swiftlanefile(lanes: [
...
/// }
/// ```
public struct Swiftlanefile: Sendable {
    /// The lanes defined in this manifest.
    public let lanes: [Lane]

    /// Creates a Swiftlanefile with the given lanes.
...
extension Swiftlanefile: Codable {
    enum CodingKeys: String, CodingKey {
        case lanes
    }

    /// Decodes a Swiftlanefile from metadata only.
    ///
    ...
    /// Encodes the Swiftlanefile as lane metadata.
    ///
