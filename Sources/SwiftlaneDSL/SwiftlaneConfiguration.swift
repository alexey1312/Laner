...
/// A protocol for user-defined Swiftlane configuration files.
///
/// Implement this protocol in your `Swiftlanefile.swift` to define
...
/// import SwiftlaneDSL
///
/// struct MySwiftlanefile: SwiftlaneConfiguration {
...
public enum ConfigurationError: Error, LocalizedError {
    ...
    /// The Swiftlanefile could not be loaded.
    case loadFailed(String)

    public var errorDescription: String? {
        switch self {
        case .laneNotFound(let name):
            return "Lane '\(name)' not found. Run 'swiftlane lanes' to see available lanes."
        case .duplicateLaneName(let name):
            return "Duplicate lane name '\(name)'. Lane names must be unique."
        case .loadFailed(let reason):
            return "Failed to load Swiftlanefile: \(reason)"
        }
    }
}
