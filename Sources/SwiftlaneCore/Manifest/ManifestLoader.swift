...
/// Loads and manages Swiftlanefile manifests.
///
/// `ManifestLoader` is responsible for finding, compiling, and executing
/// Swiftlanefile.swift files. It handles the manifest lifecycle from
...
/// 4. Parse the JSON output into a `Swiftlanefile`
///
/// ## Example Usage
///
/// ```swift
...
public struct ManifestLoader: Sendable {
    ...
    private static let manifestFileName = "Swiftlanefile.swift"

    /// The directory name containing the manifest.
    private static let manifestDirectoryName = "Swiftlane"
    ...
    public init(
        ...
        logger: Logger = Logger.swiftlane(.manifest)
    ) {
        self.shell = shell
        self.logger = logger
    }
    ...
    /// 4. Parses the output into a `Swiftlanefile`
    ///
    /// - Parameter directory: The project directory containing the manifest.
    /// - Returns: The loaded `Swiftlanefile` configuration.
    /// - Throws: `ManifestError` if loading fails at any stage.
    public func load(from directory: URL) async throws -> Swiftlanefile {
        ...
        // Execute and parse
        let swiftlanefile = try await execute(manifest: executablePath)
        logger.info("Successfully loaded manifest with \(swiftlanefile.lanes.count) lane(s)")
    ...
    private func compile(manifest: URL) async throws -> URL {
        // Determine Swiftlane installation path for linking
        ...
        // Compile the manifest
        // Link against SwiftlaneDSL from the installation
    ...
    /// - Returns: The parsed `Swiftlanefile`.
    /// - Throws: `ManifestError.executionFailed` or `ManifestError.invalidOutput`.
    private func execute(manifest: URL) async throws -> Swiftlanefile {
        let result = try await shell.run(
            manifest.path,
            arguments: [],
            timeout: .seconds(10)
        ...
        do {
            ...
            return try decoder.decode(Swiftlanefile.self, from: jsonData)
        } catch {
