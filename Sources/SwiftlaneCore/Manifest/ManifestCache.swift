...
/// │   └── SwiftlaneManifest     # Compiled executable
/// ├── <project-hash-2>/
/// │   ├── manifest.json
/// │   └── SwiftlaneManifest
/// └── ...
...
public struct ManifestCache: Sendable {
    /// The root cache directory (default: ~/.swiftlane/cache/)
    public let cacheDirectory: URL

    /// Name of the compiled executable file
    private let executableName = "SwiftlaneManifest"

    /// Name of the metadata file
    private let metadataFileName = "manifest.json"

    /// Creates a new manifest cache.
    /// - Parameter cacheDirectory: Custom cache directory. Defaults to ~/.swiftlane/cache/
    public init(cacheDirectory: URL? = nil) {
        if let cacheDirectory {
            ...
            self.cacheDirectory = homeDirectory
                .appendingPathComponent(".swiftlane", isDirectory: true)
                .appendingPathComponent("cache", isDirectory: true)
        }
    }

    /// Gets a cached manifest if it exists and the source hasn't changed.
    /// - Parameter manifestPath: Absolute path to the Swiftlanefile.swift
    ...
    ///
    /// The hash includes:
    /// - Content of Swiftlanefile.swift
    /// - Content of all .swift files in SwiftlaneHelpers/ (if directory exists)
    ///
    /// - Parameter manifestPath: Absolute path to the Swiftlanefile.swift
    ...
    public func computeHash(for manifestPath: URL) throws -> String {
        ...
        // Check for SwiftlaneHelpers directory
        let manifestDir = manifestPath.deletingLastPathComponent()
        let helpersDir = manifestDir.appendingPathComponent("SwiftlaneHelpers", isDirectory: true)
