...
/// Executes a named lane from the Swiftlanefile.
///
...
public struct LaneCommand: AsyncParsableCommand {
    ...
    public func run() async throws {
        ...
        do {
            ...
            switch error {
            case .notFound:
                print("Error: Swiftlanefile.swift not found.")
                print("Run 'swiftlane init' to create a new manifest.")
                throw ExitCode.failure
            case .compilationFailed(let output):
                print("Error: Failed to compile Swiftlanefile.swift")
                print("")
                print(output)
                throw ExitCode.failure
            ...
            case .installPathNotFound:
                print("Error: Could not determine Swiftlane installation path")
                throw ExitCode.failure
            }
        }
        ...
        guard let lane = manifest.lane(named: laneName) else {
            ...
            print("Error: Lane '\(laneName)' not found. Run 'swiftlane lanes' to see available lanes.")
            throw ExitCode.failure
        }
