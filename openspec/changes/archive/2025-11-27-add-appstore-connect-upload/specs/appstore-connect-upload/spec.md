...
Swiftlane App Store Connect Upload provides IPA upload to TestFlight and basic build management. Uses modern Build Upload API v4.1+ with chunked uploads and API Key authentication.
...
- **GIVEN** `swiftlane upload testflight --ipa App.ipa`
...
- **GIVEN** `swiftlane upload testflight --ipa App.ipa --groups "QA" --changelog "Fixes"`
- **WHEN** command executes
...
- **GIVEN** macOS system
- **WHEN** `swiftlane upload testflight` is executed
...
- **GIVEN** `swiftlane upload` command
