...
iOS developers need to upload builds to TestFlight for beta testing. Fastlane's approach relies on private Apple APIs (Spaceship) that break frequently, has 2FA/authentication issues, and requires Ruby. Swiftlane already has `AppStoreConnectAPI` for certificates/profiles but lacks build upload and TestFlight distribution.
...
- [+] `swiftlane upload testflight` CLI command
<!-- /DELTA -->
...
1. **Extend SwiftlaneMatch** with build upload capabilities (reuse existing API client)
2. **Build upload** via Build Upload API v4.1+ (chunked uploads, direct S3 upload)
3. **pilot() DSL action** - upload to TestFlight
4. **CLI command** - `swiftlane upload testflight`
...
decisions[4]{decision,choice,rationale}:
  ...
  Module,Extend SwiftlaneMatch,Reuse existing JWTGenerator and API client
...
mvp_scope[6]{feature,description}:
  ...
  CLI upload,swiftlane upload testflight command
```
...
dependencies[2]{module,reason}:
  SwiftlaneMatch,Reuse JWTGenerator and AppStoreConnectAPI
  SwiftlaneDSL,Action protocol for pilot()
```

## Success Criteria

1. Upload IPA to TestFlight via `swiftlane upload testflight`
