...
- **WHEN** user runs `laner match nuke --type distribution`
...
- **GIVEN** user runs `laner match nuke --type development`
...
- **AND** creates `Laner/MatchConfig.swift`
...
- **GIVEN** valid match configuration
- **WHEN** user runs `laner match register --name "iPhone" --udid "00001111..."`
