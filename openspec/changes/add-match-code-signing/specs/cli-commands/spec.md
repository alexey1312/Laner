...
The system SHALL provide a `swiftlane match` command group for code signing management.
...
- **WHEN** user runs `swiftlane match sync --type adhoc --app-identifier com.example.app`
- **THEN** only profiles for specified app are synced

#### Scenario: Match sync readonly

- **WHEN** user runs `swiftlane match sync --type development --readonly`
...
- **THEN** interactive prompts collect:
  ...
  - Team ID
  - App identifiers
- **AND** `Swiftlane/MatchConfig.swift` is created
...
- **WHEN** user runs `swiftlane match register --devices devices.txt`
...
- **WHEN** user runs `swiftlane match --help`
...
- **WHEN** user runs `swiftlane match sync`
