...
Code signing management is critical for iOS app distribution. Fastlane Match pioneered the "shared identity" approach where one certificate is shared across the team via encrypted Git storage. Swiftlane needs this capability to replace Fastlane.
...
swiftlane match init

# Sync certificates (download & install)
swiftlane match sync --type development
swiftlane match sync --type appstore --readonly

# Register devices
swiftlane match register --devices devices.txt

# Revoke all certificates (dangerous!)
swiftlane match nuke --type development
swiftlane match nuke --type distribution

# Change repository password
swiftlane match change-password
```
