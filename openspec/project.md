...
## Purpose
**Swiftlane** — open-source Swift CLI tool for iOS CI/CD, designed as a complete replacement for Fastlane.
...
- Modular design with clear dependency graph:
  - `swiftlane` (CLI executable) → `SwiftlaneCore`
  - `SwiftlaneCore` → `SwiftlaneDSL`, `SwiftlaneKit`
  - `SwiftlaneDSL` → `SwiftlaneKit`
  - Plugins → `SwiftlanePluginKit` → `SwiftlaneDSL`, `SwiftlaneKit`
