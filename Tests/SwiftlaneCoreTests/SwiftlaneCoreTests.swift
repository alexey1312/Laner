...
// MARK: - SwiftlaneExitCode Tests

@Suite("SwiftlaneExitCode Tests")
struct SwiftlaneExitCodeTests {
    @Test("success is 0")
    func successIsZero() {
        #expect(SwiftlaneExitCode.success.rawValue == 0)
    }

    @Test("failure is 1")
    func failureIsOne() {
        #expect(SwiftlaneExitCode.failure.rawValue == 1)
    }

    @Test("buildFailure is 2")
    func buildFailureIsTwo() {
        #expect(SwiftlaneExitCode.buildFailure.rawValue == 2)
    }

    @Test("testFailure is 3")
    func testFailureIsThree() {
        #expect(SwiftlaneExitCode.testFailure.rawValue == 3)
    }

    @Test("invalidUsage is EX_USAGE (64)")
    func invalidUsageIsExUsage() {
        #expect(SwiftlaneExitCode.invalidUsage.rawValue == 64)
    }

    @Test("configurationError is 78")
    func configurationErrorIs78() {
        #expect(SwiftlaneExitCode.configurationError.rawValue == 78)
    }
}
...
struct ManifestCacheTests {
    ...
    func defaultCacheDirectoryIsInHomeDirectory() {
        ...
        #expect(cache.cacheDirectory.path(percentEncoded: false).contains(".swiftlane/cache"))
    }
    ...
    func computeHashForNonexistentFileThrows() throws {
        let cache = ManifestCache()
        let nonexistent = URL(fileURLWithPath: "/nonexistent/path/Swiftlanefile.swift")
