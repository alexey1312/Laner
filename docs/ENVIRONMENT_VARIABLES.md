# SwiftlaneMatch Environment Variables

This document lists all environment variables recognized by SwiftlaneMatch for configuring code signing with Match.

## Overview

SwiftlaneMatch can be configured using environment variables, making it ideal for CI/CD environments. All configuration can be provided through `MatchConfiguration.fromEnvironment()` or by directly passing values to the `match()` DSL function.
...
# Test configuration
swiftlane doctor  # Check environment setup
```
...
- [SwiftlaneMatch API Documentation](./SwiftlaneMatch.swift)
